    ...
    <div v-for="message in messages" v-bind:key="message.id" class="row card mt-3">
        ...
        <div class="col-2">
          <div class="bg-light w-50 d-flex flex-column align-items-center ms-4 py-2 px-4 border rounded-pill shadow">
            <span class="py-1 px-1">
              <a @click="toggleModal(message.id)" v-if="userId == message.UserId" data-bs-toggle="tooltip" data-bs-placement="right" title="Modifier">
            ...
            <span class="py-1 px-1">
              <a @click="likes(message.id)" data-bs-toggle="tooltip" data-bs-placement="right" title="Like" >
      ...
      <div class="row d-flex py-2 border-top">
        ...
        <div class="col-3 mx-auto">
          <a @click="postComment(message.id)" class="btn btn-primary rounded-pill my-2">Publier</a>
        </div>
        <div v-for="comment in comments" v-bind:key="comment.id" class="bg-light border rounded my-2 mx-2 py-2 d-flex flex-wrap">
          <div class="col-4">
            <span>
              <img :src="comment.User.imageProfileUrl" class="rounded-pill mb-1" alt="Photo de profil" style="width: 2rem; height: 2rem;"> <br> {{ comment.User.firstName }} {{ comment.User.lastName }}
            </span> 
          </div>
          <div class="col-8">
            ...
            <p v-else class="text-center border-bottom py-2 ps-2"> {{ comment.commentaire }} </p>
          </div>
          <div class="col-9 d-flex justify-content-between">
            <p class="mt-1"> {{ comment.createdAt}} </p>
            <a @click="modifComment(comment.id)" v-if="userId == comment.UserId" class="text-decoration-none text-end cursor noActive">Modifier</a>
            <a @click="annulerModifCom()" v-if="userId == comment.UserId && mode == 'modifComment'" class="text-decoration-none text-end cursor">Annuler</a>  
          </div>
          <div class="col-3">
            <span class="px-1">
